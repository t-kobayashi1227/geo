...
export function AssociationLinks({ current }: { current?: string }) {
  ...
  return (
    <div className="mx-auto grid max-w-[270px] grid-cols-1 gap-4 md:grid-cols-3 md:gap-8 md:max-w-[860px]">
      {links.map((link) => (
        <Link
          key={link.href}
          href={link.href}
          className="flex flex-col items-center gap-2 rounded-[20px] bg-brand-blue px-4 py-6 text-center transition-opacity hover:opacity-90 md:gap-2 md:rounded-[24px] md:py-4"
        >
